...
class Response {
  constructor (node, socket, headers) {
    this.node = node
    this.socket = socket
    this.statusCode = 200
    ...
    this._chunked = true
    this._reuseChunkHeader = node._reuseChunkHeader
    this._reuseChunk = node._reuseChunk
  }
  ...
  _writeHeader (buf, n, cb) {
    ...
    if (node._host) {
      ret = requestline(node._method, node._path)
    } else {
      ret = httpStatus[this.statusCode]
    }

    this.socket.writev(
      [ret, this._headers, buf],
      [ret.length, this._headersLength + 2, n],
      cb
    )
  }
  ...
  _writeHeaderv (bufs, ns, cb) {
    ...
    this.socket.writev(
      [ret, this._headers].concat(bufs),
      [ret.length, this._headersLength + 2].concat(ns),
      cb
    )
  }
  ...
  _writeHeaderChunkedv (bufs, ns, cb) {
    ...
    const chunkHeader = node._allocSmall()
    const chunkHeaderLength = encodeHex(addAll(ns), chunkHeader)

    this.socket.writev(
      [ret, this._headers, chunkHeader].concat(bufs, EOL_BUFFER),
      [ret.length, this._headersLength + 2, chunkHeaderLength].concat(ns, 2),
  ...
  _writeHeaderChunked (buf, n, cb) {
    ...
    const chunkHeader = this.node._allocSmall()
    const chunkHeaderLength = encodeHex(n, chunkHeader)

    this.socket.writev(
      [ret, this._headers, chunkHeader, buf, EOL_BUFFER],
      [ret.length, this._headersLength + 2, chunkHeaderLength, n, 2],
      cb || this._reuseChunkHeader
    )
  }
  ...
  _writeChunk (buf, n, cb) {
    const header = this.node._allocSmall()
